...
export function SessionsView() {
  const { persistedSessions, setPersistedSessions, sessionId, sessionFile, setActiveView } = useAppStore();
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {persistedSessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            ...
            <p className="text-xs mt-2">Start a chat — sessions auto-save to ~/.pi/agent/sessions/</p>
          </div>
        ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5 flex gap-2 items-center">
                          ...
                          {session.messageCount !== undefined && <span>· {session.messageCount} msgs</span>}
                        </div>
                        {session.path && (
                          <div className="text-xs text-muted-foreground/70 mt-1 truncate font-mono">{session.path}</div>
                        )}
                      </div>
                    </div>
                  </button>
